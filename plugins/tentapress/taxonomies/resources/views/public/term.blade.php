    ...
    <body class="bg-white text-slate-900">
        <main class="mx-auto max-w-4xl space-y-10 px-6 py-12">
            <header class="space-y-3">
                <div class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{{ $taxonomy->label }}</div>
                <h1 class="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">{{ $term->name }}</h1>
            ...
            @if ($posts->count() === 0)
                <p class="text-sm text-slate-500">No posts found for this term.</p>
            @else
                <div class="space-y-6">
