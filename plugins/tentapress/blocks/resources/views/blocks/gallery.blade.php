                    ...
                    @php
                        ...
                        if (is_object($resolver) && method_exists($resolver, 'resolveImage')) {
                            $imageRef = $resolver->resolveImage(
                                ['url' => $url, 'alt' => $alt],
                                ['variant' => 'medium', 'sizes' => '(min-width: 1280px) 360px, (min-width: 768px) 33vw, 50vw']
                            );
                        }
                        ...
                        $frameClass = 'overflow-hidden border border-black/10 bg-white';
                        if ($rounded) {
                            $frameClass .= ' rounded-lg';
                        }
                    @endphp
                    <figure class="{{ $frameClass }}">
                        <div class="overflow-hidden bg-slate-50 {{ $aspectClass }}">
                            <img
                                src="{{ $imageSrc }}"
                                alt="{{ $imageAlt }}"
