    ...
    <div class="tp-metabox">
        <div class="tp-metabox__title">
            <form
                ...
                <div class="flex gap-2">
                    <a
                        href="{{ route('tp.posts.index', ['status' => 'all', 's' => $search, 'sort' => $sort, 'direction' => $direction, 'taxonomy_term' => $taxonomyTermId > 0 ? $taxonomyTermId : null]) }}"
                        class="{{ $status === 'all' ? 'tp-button-primary' : 'tp-button-secondary' }}">
                        All
                    </a>
                    <a
                        href="{{ route('tp.posts.index', ['status' => 'published', 's' => $search, 'sort' => $sort, 'direction' => $direction, 'taxonomy_term' => $taxonomyTermId > 0 ? $taxonomyTermId : null]) }}"
                        class="{{ $status === 'published' ? 'tp-button-primary' : 'tp-button-secondary' }}">
                        Published
                    </a>
                    <a
                        href="{{ route('tp.posts.index', ['status' => 'draft', 's' => $search, 'sort' => $sort, 'direction' => $direction, 'taxonomy_term' => $taxonomyTermId > 0 ? $taxonomyTermId : null]) }}"
                ...
                <div class="flex gap-2">
                    @if ($taxonomyTermOptions !== [])
                        <label class="sr-only" for="posts-taxonomy-term">Filter by taxonomy term</label>
                        <select id="posts-taxonomy-term" name="taxonomy_term" class="tp-select w-full sm:w-64">
